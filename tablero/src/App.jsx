...
export default function App() {
  ...
  const handleFile = useCallback((file) => {
    if (!file || !file.name.endsWith(".csv")) { setError("Solo se aceptan archivos .csv"); return; }
    const reader = new FileReader();
    reader.onload = (e) => {
      try {
        const parsed = parseCSV(e.target.result);
        if (!parsed.length || !parsed[0].title) throw new Error();
        setData(parsed); setFileName(file.name); setError(null);
      } catch { setError("Error al leer el CSV. Revisa que use el formato correcto."); }
    };
  ...
  return (
    <div style={{ background: "linear-gradient(160deg,#0c1322 0%,#162032 100%)", minHeight: "100vh", padding: "24px 28px", fontFamily: "'Trebuchet MS', sans-serif", color: "#e2e8f0" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "20px", borderBottom: "1px solid #1e3a5f", paddingBottom: "14px" }}>
        ...
        <button onClick={downloadTemplate} style={{ background: "#0f1f33", border: "1px solid #1e3a5f", color: "#4a7fa5", padding: "7px 14px", borderRadius: "8px", fontSize: "11px", cursor: "pointer" }}>
      ...
      <div
        ...
        style={{ border: `2px dashed ${dragging ? "#38bdf8" : "#1e3a5f"}`, borderRadius: "12px", padding: "18px", marginBottom: "20px", background: dragging ? "#38bdf810" : "#0a1628", textAlign: "center", transition: "all 0.2s", cursor: "pointer" }}
      >
        <input id="csv-input" type="file" accept=".csv" style={{ display: "none" }} onChange={e => handleFile(e.target.files[0])} />
        <div style={{ fontSize: "22px", marginBottom: "4px" }}>📂</div>
        <div style={{ fontSize: "13px", color: "#4a7fa5" }}>{dragging ? "Suelta el archivo aquí" : "Arrastra tu CSV aquí o haz clic para seleccionarlo"}</div>
        <div style={{ fontSize: "11px", color: "#2a4a65", marginTop: "4px" }}>Exporta tu Excel como .csv y súbelo — los datos se actualizan al instante</div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "20px" }}>
        {METRICS.map(m => (
          <button key={m.key} onClick={() => setActive(m.key)} style={{ padding: "6px 14px", borderRadius: "24px", cursor: "pointer", border: `2px solid ${active === m.key ? m.color : "#1e3a5f"}`, background: active === m.key ? m.color + "22" : "#0f1f33", color: active === m.key ? m.color : "#4a7fa5", fontSize: "12px", fontWeight: active === m.key ? 600 : 400 }}>
            {m.label}
          </button>
        ))}
      </div>
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div style={{ flex: 1, height: "26px", background: "#162032", borderRadius: "5px", position: "relative", overflow: "hidden" }}>
                  <div style={{ width: `${pct}%`, height: "100%", borderRadius: "5px", background: `linear-gradient(90deg,${metric.color}55,${metric.color})`, transition: "width 0.45s ease", minWidth: item[active] > 0 ? "5px" : "0", display: "flex", alignItems: "center", justifyContent: "flex-end", paddingRight: "7px" }}>
                    {pct > 18 && <span style={{ fontSize: "11px", fontWeight: 700, color: "#0c1322" }}>{item[active]}</span>}
                  </div>
                  {pct <= 18 && <span style={{ position: "absolute", left: `${pct + 1.5}%`, top: "50%", transform: "translateY(-50%)", fontSize: "11px", color: "#4a7fa5" }}>{item[active]}</span>}
                </div>
              </div>
            );
          })}
